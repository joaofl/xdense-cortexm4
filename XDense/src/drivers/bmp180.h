...
#include "math.h"
		...
		double c5,c6,mc,md,x0,x1,x2,y00,y01,y02,p0,p1,p2;
		...
		twi_packet_t i2c_pck_bmp180;
		uint8_t data_buffer_bmp180[10];
		//uint8_t _error;


		char SFE_BMP180_begin(void);
		...
		char SFE_BMP180_getTemperature_float(float *T);
			// return temperature measurement (in FLOAT) from previous startTemperature command
			// places returned value in T variable (deg C)
			// returns 1 for success, 0 for fail
		...
		char SFE_BMP180_getPressure_float(float * P, float * T);
			// return absolute pressure measurement (in FLOAT) from previous startPressure command
			// note: requires previous temperature measurement in variable T
			// places returned value in P variable (mbar)
			// returns 1 for success, 0 for fail
